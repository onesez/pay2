...
<div class="title02">修改角色</div>
    <form id="rzform">
		<input name="id" type="hidden" value="<?php echo ($editlist["id"]); ?>"/>
        <table class="V_tb2">
          <tr class="td1">
            <th>角色名称：</th>
            <td>
				<input type="text" name="title" class="V_input3" id="title" value="<?php echo ($editlist["title"]); ?>"/>
			</td>
          </tr>
          <tr class="td1">
            <th>角色描述：</th>
            <td><input type="text" name="role_desc" class="V_input3" id="role_desc" value="<?php echo ($editlist["role_desc"]); ?>"/></td>
          </tr>
          <tr class="td1">
            <th>角色权限：</th>
            <td>
					...
					<?php if(is_array($roleList)): foreach($roleList as $k=>$vo): ?><tr>
							...
							<tr>
								<td>
									...
									<?php if(is_array($vo["child"])): foreach($vo["child"] as $kk=>$vv): ?><li><input type="checkbox" name="rules[]" value="<?php echo ($vv["id"]); ?>" <?php if(in_array($vv['id'],$this_role)): ?>checked<?php endif; ?>/><?php echo ($vv["title"]); ?></li><?php endforeach; endif; ?>
...
<script type="text/javascript">  
	$(function () {  
		var ajaxurl="<?php echo U('Role/edit');?>";
