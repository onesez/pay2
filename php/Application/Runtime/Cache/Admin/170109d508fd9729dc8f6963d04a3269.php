...
<script type="text/javascript" src="/pay2/Public/js/myAjax.js"></script>
		...
		<form id="search_form" action="<?php echo U('Draw/lst');?>" method="POST">
				...
				<tr >
					<td>
						...
						<div class="clear:both;"></div>
							...
							<a class="Annkeld1" href="<?php echo U('Draw/expDraw');?>">导出搜索结果</a>
					</td>
				</tr>
			</table>
		</form>	
            ...
            <table id="dbTable" class="ipList" width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<?php if(is_array($memberlist)): foreach($memberlist as $key=>$vo): ?><tr id="<?php echo ($vo["id"]); ?>" class="td1">
					...
					<td><?php echo (msubstr($vo["reason"],0,8,'utf-8',true)); ?></td>
 					<?php if(($txcz == '1')): ?><td>
						<?php if(($vo['status'] != '被拒绝') and ($vo['status'] != '已支付')): ?><a class="smallbtn" href="javascript:;" onclick="pass('<?php echo U('Draw/pass',array('id'=>$vo['id']));?>')">处理</a>
					    <a class="smallbtn" href="<?php echo U('Draw/pay2',array('id'=>$vo['id']));?>" >代付</a><?php endif; ?>
						
						<?php if(($vo['status'] != '被拒绝')): ?><a class="smallbtn" onclick="verify(this)" data-url="<?php echo U('Draw/verify');?>" data-id="<?php echo ($vo["id"]); ?>">复核</a>	
							<?php if(($vo['success_result'] == '代付失败') and ($vo['fail_state'] != '1')): ?><a class="smallbtn" onclick="reject(this)" data-url="<?php echo U('Draw/reject');?>" data-id="<?php echo ($vo["id"]); ?>">退汇</a><?php endif; endif; ?>
					</td><?php endif; ?>					
...
function toDiv(zd_id){
	...
	var ajaxurl="<?php echo U('Draw/tajax');?>";
	$.ajax({
		url: ajaxurl,
		data : {id:zd_id},
		dataType: "json",  
...
$(function(){
    $("#dbTable tr ").dblclick(function() {
		...
		var tar_url="<?php echo U('Draw/detail');?>?id="+tar_id;	
