...
<script type="text/javascript" src="/pay2/Public/js/myAjax.js"></script>
        <div class="title02">公告列表</div>
 		<form id="search_form" action="<?php echo U('Gonggao/lst');?>" method="POST">
        ...
        <div class="block04">
            <table id="dbTable" class="ipList" width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<?php if(is_array($memberlist)): foreach($memberlist as $key=>$vo): ?><tr id="<?php echo ($vo["id"]); ?>" class="td1">
					...
					<td><?php echo (date('Y-m-d H:i:s',$vo['create_date']?$vo['create_date']:'')); ?></td>
					<td><?php echo ($vo["title"]); ?></td>			
					<td><?php echo (msubstr($vo["content"],0,100,'utf-8',true)); ?></td>
					<td>
						<a href="<?php echo U('Gonggao/edit',array('id'=>$vo['id']));?>" class="smallbtn">修改</a>						
						<a class="smallbtn" onclick="delfunc(this)" data-url="<?php echo U('Gonggao/del');?>" data-id="<?php echo ($vo["id"]); ?>">删除</a>
					</td>
	   ...
	   return false;
		...
		layer.confirm('确认删除？', {
			  btn: ['确定','取消'] //按钮
				...
				var ajaxurl="<?php echo U('Gonggao/delBatch');?>";
				$.ajax({
					type:"POST",
					url:ajaxurl,
					data:{key:chk_value},
...
$(function(){
    $("#dbTable tr ").dblclick(function() {
		...
		var tar_url="<?php echo U('Gonggao/detail');?>?id="+tar_id;	
