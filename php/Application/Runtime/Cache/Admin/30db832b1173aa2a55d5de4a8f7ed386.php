        ...
        <div class="block04">
            <table id="dbTable" class="ipList" width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<?php if(is_array($memberlist)): foreach($memberlist as $key=>$vo): ?><tr id="<?php echo ($vo["id"]); ?>" class="td1">
					...
					<td>						
						<a href="<?php echo U('Platform/detail',array('id'=>$vo['id']));?>" class="smallbtn">详情</a>
						<a href="<?php echo U('Platform/edit',array('id'=>$vo['id']));?>" class="smallbtn">编辑</a>						
						<a class="smallbtn" onclick="delfunc(this)" data-url="<?php echo U('Platform/del');?>" data-id="<?php echo ($vo["id"]); ?>">删除</a>
					</td>
	   ...
	   return false;
		...
		layer.confirm('确认删除？', {
			  btn: ['确定','取消'] //按钮
				...
				var ajaxurl="<?php echo U('Platform/delBatch');?>";
				$.ajax({
					type:"POST",
					url:ajaxurl,
					data:{key:chk_value},
